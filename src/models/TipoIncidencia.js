...
class TipoIncidencia {
	 ...
	 */
	static async getAll() {
        const query=`select t.multitabla_id,t.tabla,t.sigla,t.valor,t.padre_id,t.estado,c.valor clasificacion from ssc_multitabla t INNER JOIN ssc_multitabla c on t.padre_id=c.multitabla_id where t.tabla=$1 and c.tabla=$2  ORDER BY 1;`
	...
	static async getIncidentByReporterName(name) {
        name+='%';
		const queryResult = await pool.query(`select * from ssc_multitabla where tabla="ssc_tipo_inc" AND valor LIKE $1`, [name]);
		if (queryResult) {
			const result = queryResult.rows[0];
			if (result) {
				return result;
	 ...
	 */
	 static async getAllTipoByIdClasificacion(id) {
        const query=`select t.multitabla_id,t.tabla,t.sigla,t.valor,t.padre_id,t.estado,c.valor clasificacion from ssc_multitabla t INNER JOIN ssc_multitabla c on t.padre_id=c.multitabla_id where t.tabla=$1 and c.tabla=$2 and t.padre_id=$3 ORDER BY 4;`
	...
	static async delete(id) {
		const queryResult = await pool.query(`DELETE from ssc_multitabla where multitabla_id =$1 AND tabla ='ssc_tipo_inc'`, [id]);
		if (queryResult) {
			const result=queryResult.rows[0];
			if (result) {
				return result;
